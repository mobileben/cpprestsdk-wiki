...
If the selections on your screen match the screenshot and there are still "No packages found" when searching for casablanca, then it might be an issue with NuGet. Please check [here](http://docs.nuget.org/release-notes/known-issues#error-installing-packages-with-nuget-3.4,-3.4.1) for resolving the issue. 
